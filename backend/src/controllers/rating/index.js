...
const AddRating = async (req, res) => {
    ...
    if (!user_id || !movie_id || !ratingValue) { return res.sendStatus(400) };
    ...
    try {
        ...
        const foundRating = await ratingList.rating.find(pair => pair.movie_id === JSON.stringify(movie_id));
...
const DeleteRating = async (req, res) => {
    ...
    if (!user_id || !movie_id) { return res.sendStatus(400) };
    ...
    try {
        ...
        const filteredList = ratingList.rating.filter(pair => pair.movie_id !== JSON.stringify(movie_id));
...
const GetRating = async (req, res) => {
    ...
    if (!user_id || !movie_id) { res.sendSatus(400) }

    try {

        const ratingList = await Rating.findOne({ user_id: user_id });
        const rating = ratingList.rating.find(rating => (rating.movie_id === movie_id));
